...
public class RenderCustomLiquid implements ISimpleBlockRenderingHandler {
    ...
    public static RenderCustomLiquid instance = new RenderCustomLiquid();
    ...
    public float getFluidHeightForRender(IBlockAccess world, int x, int y, int z, BlockFluidBase block) {
        ...
        if (world.getBlock(x, y, z) == block) {
            ...
            if (world.getBlock(x, y - densityDir, z).getMaterial().isLiquid()) {
                return 1;
            }
        ...
        return !world.getBlock(x, y, z).getMaterial().isSolid() && world.getBlock(x, y - densityDir, z) == block ? 1 : block.getQuantaPercentage(world, x, y, z) * 0.875F;
    }
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
        ...
        boolean renderTop = world.getBlock(x, y - densityDir, z) != theFluid;
        ...
        if (!renderTop && !renderBottom && !renderSides[0] && !renderSides[1] && !renderSides[2] && !renderSides[3]) {
            ...
            if (renderer.renderAllFaces || renderTop) {
                rendered = true;
                IIcon iconStill = getIcon(block.getIcon(world, x, y, z, 1));
                float flowDir = (float) BlockFluidBase.getFlowDirection(world, x, y, z);

                if (flowDir > -999.0F) {
                    iconStill = getIcon(block.getIcon(world, x, y, z, 2));
                }
            ...
            if (renderer.renderAllFaces || renderBottom) {
                ...
                if (!rises) {
                    ...
                    renderer.renderFaceYNeg(block, x, y + RENDER_OFFSET, z, getIcon(block.getIcon(world, x, y, z, 0)));
                } else {
                    tessellator.setColorOpaque_F(LIGHT_Y_POS * red, LIGHT_Y_POS * green, LIGHT_Y_POS * blue);
                    renderer.renderFaceYPos(block, x, y + RENDER_OFFSET, z, block.getIcon(world, x, y, z, 1));
                }
            }
            ...
            for (int side = 0; side < 4; ++side) {
                ...
                IIcon iconFlow = getIcon(block.getIcon(world, x, y, z, side + 2));
                if (renderer.renderAllFaces || renderSides[side]) {
                    ...
                    float u1Flow = iconFlow.getInterpolatedU(side == 0 || side == 3 ? 0.0D : 16.0D);
                    float u2Flow = iconFlow.getInterpolatedU(side == 0 || side == 3 ? 16.0D : 0.0D);
                    float v1Flow = iconFlow.getInterpolatedV((1.0D - ty1) * 16.0D );
                    float v2Flow = iconFlow.getInterpolatedV((1.0D - ty2) * 16.0D );
                    float v3Flow = iconFlow.getInterpolatedV(16.0D);
